...
GST_START_TEST (test_pipeline_play_success)
{
  GstcStatus ret;
  const gchar *pipeline_name = "pipe";
  const gchar *expected = "update /pipelines/pipe/state playing";

  ret = gstc_pipeline_play (_client, pipeline_name);
...
GST_START_TEST (test_pipeline_play_null_client)
{
  GstcStatus ret;
  const gchar *pipeline_name = "pipe";

  ret = gstc_pipeline_play (NULL, pipeline_name);
  assert_equals_int (GSTC_NULL_ARGUMENT, ret);
}
...
libgstc_pipeline_suite (void)
{
  ...
  tcase_add_test (tc, test_pipeline_play_success);
  tcase_add_test (tc, test_pipeline_play_null_name);
  tcase_add_test (tc, test_pipeline_play_null_client);
