...
GST_START_TEST (test_pipeline_pause_success)
{
  GstcStatus ret;
  const gchar *pipeline_name = "pipe";
  const gchar *expected = "update /pipelines/pipe/state paused";

  ret = gstc_pipeline_pause (_client, pipeline_name);
...
GST_START_TEST (test_pipeline_pause_null_client)
{
  GstcStatus ret;
  const gchar *pipeline_name = "pipe";

  ret = gstc_pipeline_pause (NULL, pipeline_name);
  assert_equals_int (GSTC_NULL_ARGUMENT, ret);
}
...
libgstc_pipeline_suite (void)
{
  ...
  tcase_add_test (tc, test_pipeline_pause_success);
  tcase_add_test (tc, test_pipeline_pause_null_name);
  tcase_add_test (tc, test_pipeline_pause_null_client);
