...
{
  GstcStatus ret;
  const gchar *expected = "read /pipelines";
  char **response;
  int array_lenght;
...
GST_START_TEST (test_pipeline_list_null_client)
{
  GstcStatus ret;
  char **response;
  int array_lenght;
...
GST_START_TEST (test_pipeline_list_null_pipelines)
{
  GstcStatus ret;
  char ***response = NULL;
  int array_lenght;
...
static Suite *
libgstc_pipeline_list_suite (void)
{
  Suite *suite = suite_create ("libgstc_pipeline_list");
  ...
  tcase_add_test (tc, test_pipeline_list_null_client);
  tcase_add_test (tc, test_pipeline_list_null_pipelines);

  return suite;
}

GST_CHECK_MAIN (libgstc_pipeline_list);
