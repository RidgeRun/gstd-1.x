...
typedef enum
{
  ...
  gstd_SEND_ERROR = -8,
  gstd_RECV_ERROR = -9,
  gstd_THREAD_ERROR = -11,
  gstd_BUS_TIMEOUT = -12,
  gstd_SOCKET_TIMEOUT = -13,
  gstd_LONG_RESPONSE = -14
} GstdStatus;
 ...
 */
GstdStatus 
gstd_manager_new (GstDManager ** manager, Supported_IPCs * ipc[]);
 ...
 */
GstdStatus
gst_pipeline_get_state (GstDManager ** manager, const char *pipeline_name,
    char **out);
